...
def integrate2d_sin_sin(A,B,C,D,a,b,c,d):
	...
	Cos(a *B - B* c + b *C - C *d))) / (a - c)) + ((b + d)*(Cos(a* A - A *c + b *D - d *D) -
	Cos(a *B - B *c + b *D - d *D))) / (a - c) + (1 / (
			a + c))*(b - d)*(Cos(A*(a + c) + C*(b + d)) - Cos(B*(a + c) + C*(b + d)) - Cos(A*(a + c) + (b + d)*
	D) + Cos(B*(a + c) + (b + d)*D)))
	return val
...
def integrate2d_sin_cos(A,B, C, D, a,b, c,d):
	...
	val = (1/(2 *(b - d) *(b + d)))*(((b + d) *(-Sin(a* A - A *c + b *C - C *d) +
     Sin(a* B - B *c + b* C - C* d)))/(a - c) + ((b + d) *(Sin(a *A - A *c + b *D - d* D) -
     Sin(a *B - B* c + b *D - d *D)))/(a - c) - (1/( a + c))*(b - d)* (Sin(A *(a + c) + C *(b + d)) -
     Sin(B *(a + c) + C *(b + d)) - Sin(A *(a + c) + (b + d)* D) +
     Sin(B* (a + c) + (b + d) *D)))
	return val
...
def integrate2d_cos_cos(A,B, C, D, a,b, c,d):
   ...
   a + c))*(b - d)* (Cos(A *(a + c) + C* (b + d)) -
      Cos(B *(a + c) + C* (b + d)) - Cos(A* (a + c) + (b + d)* D) + Cos(B* (a + c) + (b + d)* D)))
	return val
...
def quadvec2(func,x1,x2,y1,y2, epsabs=1e-200, epsrel=1e-08, limit = 1000, workers =1, quadrature = 'gk21'):
	...
	3.0
	"""
	func2 = lambda y: integrate.quad_vec(lambda x: func(x, y), x1, x2,epsabs=epsabs, epsrel=epsrel, limit = limit,quadrature=quadrature)[0]
