...
def transform(X,low = -1, high = 1, functions = True, offsets = None):
	...
	trans = lambda Z: torch.stack([torch.from_numpy(np.apply_along_axis(transforms[i],0,Z[:,i].numpy())) for i in range(d)]).T
	inv_trans =lambda Y: torch.stack([torch.from_numpy(np.apply_along_axis(inv_transforms[i],0,Y[:,i].numpy())) for i in range(d)]).T
