...
class GaussianProcessFF(KernelizedFeatures):
	...
	def __init__(self, project = None, gamma=0.1, s=0.001, approx="rff", m=100, d=1, diameter=1.0, verbose=True, groups=None,
	...
	def sample_embedding(self,d_effective,m,gamma):
		...
		if self.approx == "quad":
			embedding_map = QuadratureEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "rff":
			embedding_map = RFFEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "rff2":
			embedding_map = RFFEmbedding(biased = True, gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "halton":
			embedding_map = RFFEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "hermite":
			embedding_map = HermiteEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "trapezoidal":
			embedding_map = TrapezoidalEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
			...
			embedding_map = MaternEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "quad_periodic":
			embedding_map = QuadPeriodicEmbedding(gamma=gamma, nu=self.nu, m=m, d=d_effective, diameter=self.diameter, groups=None,
				  kernel=self.kernel, approx = self.approx)
		elif self.approx == "kl":
	...
	def log_marginal_likelihood(self,gamma,Rot,kappa,kernel = "default"):
		...
		logprob = -0.5 * (torch.mm(torch.t(self.y),self.y)- torch.mm(torch.t(rhs),alpha))/self.s**2  + logdet # - 0.5*self.n*np.log(2*np.pi)
		logprob = -logprob
	...
	def sample_and_optimize(self, xtest = None, multistart=25, minimizer = "L-BFGS-B", grid = 100, verbose = 0):
		...
		for j in range(multistart):
			...
			if minimizer== "L-BFGS-B":
				...
				res = projected_gradient_descent(fun, grad, x0, mybounds, tol = 0.001, nu = 1./(self.m*np.max(np.abs(theta))))
				solution = res.x
