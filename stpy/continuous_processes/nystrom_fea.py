...
class NystromFeatures(RandomProcess):
	'''
	...
	def fit_gp(self, x, y):
		...
		if self.approx == "svd":
			...
			X = ysample
			model = NMF(n_components=self.ms, max_iter = 2000, tol=1e-8)
			W = torch.from_numpy(model.fit_transform(X))
			H = torch.from_numpy(model.components_)
			l = torch.norm(W,dim=1)
			l = 1./l
			#max_val = self.kernel(x,x)[0,0]*2
			#s = lambda q: self.kernel(q,x)/max_val
			#self.embed = lambda q: torch.t(W.T@torch.diag(l)@s(q))
			fs = []
			from scipy.interpolate import interp1d
			for j in range(self.ms):
				fs.append(interp1d(x.view(-1).numpy(), (W.T @ torch.diag(l))[j, :].numpy()))
			self.embed = lambda q: torch.cat([torch.from_numpy(fs[j](q)).view(-1, 1) for j in range(self.ms)], dim=1)
			...
			K = torch.mm(torch.mm(self.Dweights,self.kernel(xs,xs)),self.Dweights) #+ self.s*self.s * torch.eye(self.ms, dtype=torch.float64)
			(D,V) = torch.symeig(K,eigenvectors=True)
			Dinv = torch.diag(1./D)
			Dinv[Dinv <=0 ] = 0
			Dinv = torch.sqrt(Dinv)
			#Dinv = torch.diag(torch.pow(D[:],-0.5))
			self.embed = lambda q: torch.t(torch.mm(Dinv,torch.mm(torch.t(V), torch.mm(self.Dweights,self.kernel(q,xs))   )  ))
	...
	def visualize(self,xtest,f_true = None, points = True, show = True):
		...
		if self.d == 1:
			...
			if points == True:
				ax.scatter(self.x[:, 0].numpy(), self.x[:, 1].numpy(), self.y[:,0].numpy(), c='r', s=100, marker="o", depthshade=False)
