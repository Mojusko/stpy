...
class SwissFEL(BenchmarkFunction):
	def __init__(self, **kwargs):
		...
		GP = stpy.continuous_processes.gauss_procc.GaussianProcess(groups=stpy.helpers.helper.full_group(self.d), gamma=torch.ones(self.d, dtype=torch.float64), kernel="ard")
...
class GaussianProcessSample(BenchmarkFunction):
	...
	def sample(self,n):
		...
		GP = stpy.continuous_processes.gauss_procc.GaussianProcess(s=self.sigma, gamma=self.gamma, kernel = self.kernel_name)
