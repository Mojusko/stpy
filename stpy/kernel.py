...
from stpy.kernels.kernel_function_base import KernelFunctionBase
from stpy.kernels.kernel_functions.squared_exponential_kernel import squared_exponential_kernel, squared_exponential_kernel_diag
from abc import ABC

class KernelFunction(ABC):
	def __init__(self,
			  	kernel_function: Union[KernelFunctionBase,Callable,None]=None, # custom kernel function
			  	kernel_name:str ="squared_exponential", # Name of the kernel 
				freq=None, # frequency of the periodic kernel
				groups=None, # groups of the input space for additive kernels
				d: int=1, # dimension of the input space
				gamma: float=1, # gamma parameter of the squared exponential and matern kernel 
				ard_gamma: torch.Tensor=None, # ARD gamma parameter of the ARD kernel 
				nu: float=1.5, # nu parameter of the matern kernel
				kappa: float=1, # scaling parameter of the kernel
				map=None, #	embedding map for custom kernels
				power: int=2, # power of the polynomial kernel
				cov: torch.Tensor=None, # covariance matrix of the full covariance kernel
				params: Union[None,dict]=None, # initial parameters of the kernel, for custom kenrel 
				group: Union[None,List]=None, # group of the input space for additive kernels
				offset = 0. # offset of the linear kernel, to ensure a constant shift 
				):
	...
	def covar_kernel(self, a, b, **kwargs):
		...
		#a = torch.mm(a, cov)
		#b = torch.mm(b, cov)
		a = torch.linalg.lstsq(cov, a.T)[0].T
		b = torch.linalg.lstsq(cov, b.T)[0].T
