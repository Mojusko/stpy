...
from stpy.embeddings.polynomial_embedding import PolynomialEmbedding, ChebyschevEmbedding, CustomEmbedding
...
if __name__ == "__main__":
    ...
    T = 100
    d = 10
    xtest = torch.eye(d).double()

    # space of actions
    #xtest = interval_torch(n, d= 1)  # pytorch array (n, d)
    x = torch.zeros(1,d).double()

    benchmark_f = CustomBenchmark(d=1, func = lambda x: 1*x[:,1]**1)
    # scale
    ...
    #embedding = PolynomialEmbedding(kappa = kappa, d = 1, p = 20)
    embedding = CustomEmbedding(d,lambda x: np.sqrt(kappa)*x, m = d)
    m = embedding.get_m()
    ...
    base_constraint = base_regularizer.get_constraint_object(lam/(2))
    ...
    for i in range(T):
        ...
        # sample random point
        xnext = xtest[int(np.random.randint(0,d,1)),:].view(1,-1)
        ...
        plt.legend()
        plt.savefig(f"pics/figure{i:02d}.png")
        #plt.show()
